...
const CatalogoView = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Construir lista de categorías únicas desde la DB
  ...
  // Filtrado lógico por nombre de categoría
            ...
            {productosFiltrados.map((prod) => (
              <div key={prod.id} className={`group border rounded-[2.5rem] overflow-hidden transition-all duration-500 shadow-2xl ${isAdmin
                ...
                <div className={`aspect-square relative overflow-hidden flex items-center justify-center p-12 ${isAdmin ? 'bg-slate-50' : 'bg-slate-800/50'}`}>
                  ...
                  {/* Renderizado de Imagen Real */}
                  <img
                    src={
                      prod.imagen?.startsWith('http')
                        ? prod.imagen
                        : `http://localhost:5000/${prod.imagen?.replace(/\\/g, '/').replace(/^\//, '')}`
                    }
                    alt={prod.nombre}
                    className="relative z-10 w-full h-full object-contain group-hover:scale-110 transition-transform duration-500"
                    onError={(e) => {
                      // Si falla la local, intenta esta que no requiere internet para mostrar que hay un error
                      e.target.src = 'data:image/svg+xml;charset=UTF-8,%3Csvg%20width%3D%22400%22%20height%3D%22400%22%20xmlns%3D%22http%3A//www.w3.org/2000/svg%22%3E%3Crect%20width%3D%22100%25%22%20height%3D%22100%25%22%20fill%3D%22%23222%22/%3E%3Ctext%20x%3D%2250%25%22%20y%3D%2250%25%22%20font-family%3D%22sans-serif%22%20font-size%3D%2220%22%20fill%3D%22%23555%22%20text-anchor%3D%22middle%22%3EError%20Ruta%20Imagen%3C/text%3E%3C/svg%3E';
                    }}
                  />
                </div>
                ...
                <div className="p-10">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <p className="text-[9px] text-slate-500 font-mono">SKU: {prod.sku}</p>
                    </div>
                  ...
                  <p className="text-slate-500 text-sm leading-relaxed mb-8">
                    Stock disponible: <span className="text-blue-500 font-bold">{prod.stock_actual} unidades</span>. Optimizado para entornos industriales.
                  </p>
                  ...
                  <div className={`border-none outline-none rounded-[2.5rem] p-10 shadow-2xl max-w-lg w-full ${isAdmin ? 'bg-white' : 'bg-[#0d1117]'}`}>
                    ...
                    <div className={`space-y-4 text-sm border-t pt-6 ${isAdmin ? 'text-slate-600 border-slate-100' : 'text-slate-400 border-white/5'}`}>
                      <div className={`flex justify-between border-b pb-2 ${isAdmin ? 'border-slate-100' : 'border-white/5'}`}>
                        <span className="font-bold text-slate-500 uppercase text-[10px]">Costo Base</span>
                        <span className={`font-mono text-xs ${isAdmin ? 'text-slate-900' : 'text-white'}`}>${prod.costo}</span>
                      </div>
                      <div className={`flex justify-between border-b pb-2 ${isAdmin ? 'border-slate-100' : 'border-white/5'}`}>
                    ...
                    <p className="py-6 text-slate-500 italic text-xs leading-relaxed">
                      * Producto verificado bajo estándares de calidad EGYVEN. SKU: {prod.sku}
                    </p>
